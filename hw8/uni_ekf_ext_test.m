...
% extended to include wheel radius uncertainty
% - by Dimitri Lezcano
...
P = 0.01*diag([1, 1, 1, 4]); % covariance
...
% final plot saving
saveas(fig1, 'prob_3a_traj.png'); 
disp('Saved figure: prob_3a_traj.png');

saveas(fig2, 'prob_3a_err.png'); 
disp('Saved figure: prob_3a_err.png');
...
if nargout > 1
  % F-matrix
  varargout{1} = [1, 0, -s*u(1)*r, c * u(1);
                  0, 1, c*u(1)*r,  s * u(1); 
                  0, 0, 1,         0;
                  0, 0, 0,         1];
end

% beacon and range measurements
...
% Extended Kalman filtering functions
...
% state and measurement angle fixing
